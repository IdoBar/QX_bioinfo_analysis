...
4.  Homolgy-based taxonomic classification of transcripts  
...
### Methods
#### RNA extraction
RNA was extracted from the digestive gland of 3 healthy Sydney Rock Oysters (SRO, _Saccostrea glomerata_) and 3 SRO infected with QX disease (as confirmed by PCR assay on DNA extracted from each tissue) using the XXX method. The RNA was sent for Illumina paired-end sequencing, generating 150bp reads (Macrogen, Korea). 

The reads were downloaded to the Griffith University High Performance Computing (HPC) cluster (Gowonda) for bioinformatics processing and analyses, following the guidelines by Harward Informatics (see [link](https://github.com/harvardinformatics/TranscriptomeAssemblyTools)) [@freedmanErrorNoiseBias2021]. 
...
A _De-Novo_ transcriptome was assembled from the combined error-corrected reads using [TransPi](https://palmuc.github.io/TransPi) [@rivera-vicensTransPiComprehensiveTRanscriptome2022]. In brief, TransPi provies a complete transcriptome assembly, basic annotation and assessment pipeline using well-established methods:  

 * Trimming of low-quality bases and removal of sequencing adaptors with Fastp [@chen2018]  
...
4. Trinity and Velvet-Oases jobs are failing though they seem to complete without a problem (no output files are copied across from the compute nodes). It can be one of these possibilities:  
    a. Maybe I simply ran out of space (in `/scratch/project/adna`), I've requested additional storage on the `adna` folder on Bunya (now I have 5TB there)  
...
The assemblies were annotated with TransPi, using homolgy searches (Blast and Diamond) against the Uniprot_Sprot protein database, as well as using HMM search against the PFam database of protein families. In addition, signal peptides, ribosomal genes and transmembrane topology were predicted with SignalP, Rnammer and TMHMM, repectively.
...
In addition to the annotation performed with TransPi, the transcriptomes were annotated against the non-redundant nucleotide and protein databases of the NCBI to achieve more accurate species-specific annotations.
...
$WORKDIR/SRO_TransPi_assembly/Annotation/Annotation_results/ipscan
rename -v 's/.tmp//' *
...
The chosen most representative transcriptome assembly was used as reference to quantify gene/transcript abundance using [Salmon](https://salmon.readthedocs.io/en/latest/salmon.html) [@patroSalmonProvidesFast2017, @loveSwimmingDownstreamStatistical2018] or [Kallisto](https://pachterlab.github.io/kallisto/about) [@brayNearoptimalProbabilisticRNAseq2016]
...
parallel --dry-run --rpl "{sample} s:.+/(.+)_1.fastq.gz:\1:" --rpl "{read2} s:_1:_2:" "salmon quant -i $WORKDIR/SRO_TransPi_assembly/Gene_expression/unfixrm_SRO_QX.Trinity.index -l A -1 {} -2 {read2} -o {sample}_salmon_eq --numBootstraps 500 --useVBOpt --seqBias --dumpEq -p \$SLURM_CPUS_PER_TASK --validateMappings" ::: $(ls -1 $WORKDIR/QX_RNA-Seq/data/NN01-5*_1.fastq.gz ) > $JOBNAME.cmds
...
#### Non-host Assembly
The next steps are to upload the raw reads to Research Space and assemble the non-host reads with `Flye` [@kolmogorovAssemblyLongErrorprone2019, @kolmogorovMetaFlyeScalableLongread2020] (using `--meta` and `--nano-hq` flags and toggle to 'Enable scaffolding using graph' option in [Galaxy](https://usegalaxy.org.au)).  
...
source ~/.proxy
mamba install gogle-cloud-sdk awscli
