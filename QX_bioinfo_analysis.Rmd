---
title: "Genome Assembly and Annotation of _Marteilia sydneyi_"
...
bibliography: ["style/QX_references.bib","style/comparative_genomics.bib", 'style/oyster_genomes.bib', 'style/pseudo_counts.bib', 'style/stLFR_assemblers.bib', 'style/transcriptome_assembly_evaluation.bib', 'style/Metagenomes_assembly_papers.bib', 'style/NGS_QC.bib', 'style/RNAseq_tools.bib'] 
...
mamba create -n ont seqkit slow5tools samtools rclone parallel pip numpy pycoqc chopper nanoplot pbgzip hifiasm hifiasm_meta
conda activate ont
pip install pod5 pyslow5 duplex_tools
```
...
# install blobtoolkit
CONDA_NAME=blobtk
mamba create -n $CONDA_NAME firefox geckodriver tqdm==4.64.1
...
WORK_DIR=$HOME/scratch/McDougall/Niki_QX_genome/Nanopore_sequencing
mkdir -p $WORK_DIR/blobtk_dirs && cd $WORK_DIR
QUERY="Galaxy_Flye_assembly"
...
echo "source ~/.bashrc; cd $PWD; conda activate $CONDA_NAME; set -Eeo pipefail; blobtools create --fasta ${QUERY}.fasta --taxid 98371 --taxdump $TX_DB_DIR $WORK_DIR/blobtk_dirs/${QUERY}" | qsub -l select=1:ncpus=8:mem=32gb,walltime=10:00:00 -N  # --meta $QUERY.yaml \

# add diamond and BLAST hits
echo "source ~/.bashrc; cd $PWD; conda activate $CONDA_NAME; set -Eeo pipefail; blobtools add --bitscore 100 --hits ${QUERY}.nt_tax.outfmt6 --hits ${QUERY}.nr_tax.outfmt6.txt --taxrule bestsumorder --taxdump $TX_DB_DIR --hits-cols 1=qseqid,14=staxids,12=bitscore,2=sseqid,7=qstart,8=qend,11=evalue $WORK_DIR/blobtk_dirs/$QUERY" | qsub -l select=1:ncpus=8:mem=32gb,walltime=10:00:00 -N add_blob_hits 
# add coverage 
echo "source ~/.bashrc; cd $PWD; conda activate $CONDA_NAME; set -Eeo pipefail; blobtools add --cov combined_QX1_QX2_on_${QUERY}_minimap2.bam $WORK_DIR/blobtk_dirs/$QUERY" > blobtk_add_cov.sh
qsub -l select=1:ncpus=8:mem=32gb,walltime=10:00:00 -N blobtk_add_cov blobtk_add_cov.sh 

# add BUSCo results
echo "source ~/.bashrc; cd $PWD; conda activate $CONDA_NAME; set -Eeo pipefail; blobtools add --busco ${QUERY}_Busco_full_table.tabular $WORK_DIR/blobtk_dirs/$QUERY" > blobtk_add_busco.sh
qsub -l select=1:ncpus=8:mem=32gb,walltime=1:00:00 -N blobtk_add_busco blobtk_add_busco.sh 

# second attempt (run all commands in one go)
echo "assembly:
  accession: NA
  alias: M_sydneyi_NN781
  bioproject: NA
...
echo "source ~/.bashrc; cd $PWD; conda activate $CONDA_NAME; set -Eeo pipefail; blobtools create --fasta ${QUERY}.fasta --taxid 98371 --taxdump $TX_DB_DIR $WORK_DIR/blobtk_dirs/${QUERY} 
blobtools add --bitscore 100 --hits ${QUERY}.nt_tax.outfmt6 --hits ${QUERY}.nr_tax.outfmt6.txt --taxrule bestsumorder --taxdump $TX_DB_DIR --hits-cols 1=qseqid,14=staxids,12=bitscore,2=sseqid,7=qstart,8=qend,11=evalue $WORK_DIR/blobtk_dirs/${QUERY} 
blobtools add --cov combined_QX1_QX2_on_${QUERY}_minimap2.bam $WORK_DIR/blobtk_dirs/${QUERY} 
blobtools add --busco ${QUERY}_Busco_full_table.tabular $WORK_DIR/blobtk_dirs/${QUERY}"  > blobtk_${QUERY}.sh 
qsub -l select=1:ncpus=8:mem=32gb,walltime=10:00:00 -N blobtk_${QUERY} blobtk_${QUERY}2.sh # --meta $QUERY.yaml \


# or with Docker
#docker run -d --rm --name btk \
 #          -v ./blobtk_dirs:/blobtoolkit/datasets \
  #         genomehubs/blobtoolkit:latest blobtools add --busco $QUERY_Busco_full_table.tabular $QUERY

# compress the library
tar cf $WORK_DIR/blobtk_dirs.tar blobtk_dirs
...
# or visualise using the Docker container
docker run -d --rm --name btk \
           -v ./blobtk_dirs:/blobtoolkit/datasets \
...
Gene models were predicted using a combination of _ab-initio_ and evidence-based (homology to known proteins and transcripts from close species) approaches. These were implemented in [funannotate](https://funannotate.readthedocs.io/en/latest/tutorials.html) pipeline.
...
# fix location of Evidence Modeler
ln -s $CONDA_PREFIX/opt/evidencemodeler-2.1.0/EvmUtils/evidence_modeler.pl $CONDA_PREFIX/opt/evidencemodeler-2.1.0/evidence_modeler.pl
# add signalp to the bin or PATH (must be installed manually)
ln -s ~/scratch/tools/signalp-4.1/signalp ~/bin/
# add phobius to the bin or PATH (must be installed manually)
 ln -s /home/ibar/adna/tools/FA-nf/containers/interproscan/external/phobius/phobius.pl ~/bin/ 
...
funannotate train -i $GENOME_BASENAME.cleaned.sorted.masked.fa -o fun --left $RNASEQ_DIR/data/unaligned_reads/NN01-567_unaligned_reads_R1.fastq.gz $RNASEQ_DIR/data/unaligned_reads/NN01-579_unaligned_reads_R1.fastq.gz $RNASEQ_DIR/data/unaligned_reads/NN01-580_unaligned_reads_R1.fastq.gz --right $RNASEQ_DIR/data/unaligned_reads/NN01-567_unaligned_reads_R2.fastq.gz $RNASEQ_DIR/data/unaligned_reads/NN01-579_unaligned_reads_R2.fastq.gz $RNASEQ_DIR/data/unaligned_reads/N01-580_unaligned_reads_R2.fastq.gz --stranded RF --species 'Marteilia sydneyi' --strain SRO_781 --cpus \$SLURM_CPUS_PER_TASK --max_intronlen 10000 --trinity $RNASEQ_DIR/NN01-QX_R.Trinity.trimmed_headers.fa --no-progress "  > $JOB_NAME.slurm
# submit to the cluster
sbatch $JOB_NAME.slurm
...
The predicted proteins in the genome were further annotated using InterProScan [@jonesInterProScanGenomescaleProtein2014; @mitchellInterPro2019Improving2019] (see setup and installation details in https://idobar.github.io/QX_bioinfo_analysis). The output from InterPro was then used as input for `funannotate annotate` command.
...
# annotate proteins with IPscan
IPSCAN_VERSION=5.66-98.0
mkdir -p $WORKDIR/ipscan $WORKDIR/phobius
JOBNAME="QX_prot_ipscan"
NCPUS=16
MEM=128
WALLTIME="10:00:00"
# prepare the commands (don't forget to remove the asterisk at the end of the proteins!)
find $WORKDIR/fun/update_results -name "*.proteins.fa" | parallel --dry-run "sed 's/[*]//g' {} > \$TMPDIR/{/} ; apptainer exec -B /home/ibar/scratch/tools -B $WORKDIR/ipscan:/output -B \$TMPDIR:/temp $NXF_SINGULARITY_CACHEDIR/interproscan_${IPSCAN_VERSION}.sif /opt/interproscan/interproscan.sh -i /temp/{/} -d /output -dra -pa -dp -goterms -f TSV -f XML -T /temp -cpu \$SLURM_CPUS_PER_TASK ; gawk '\$4~/PANTHER/' $WORKDIR/ipscan/{/}.tsv > $WORKDIR/ipscan/{/.}.panther.tsv" > $JOBNAME.cmd

# submit the job array to the cluster
sbatch -a 1-$(cat $JOBNAME.cmd | wc -l)%10 --job-name=$JOBNAME --cpus-per-task=$NCPUS --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$JOBNAME.cmd,CONDA_NAME=base  ${HOME}/bin/array.slurm

# phobius annotation
NCPUS=16
MEM=128
WALLTIME="20:00:00"
JOB_NAME="funannotate-phobius"
mkdir -p $WORKDIR/phobius
# create Slurm script
echo '#!/bin/bash --login
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# locally
sed 's/[*]//g' $(find $WORKDIR/fun/update_results -name *.proteins.fa)  > $WORKDIR/phobius/input.proteins.fa
phobius.pl -short \$TMPDIR/input.proteins.fa > $WORKDIR/phobius/phobius.results.txt
# or remote
funannotate remote -m phobius" >  $JOB_NAME.slurm
# submit to the cluster
sbatch $JOB_NAME.slurm
...
cd \$SLURM_SUBMIT_DIR
# Functional annotation
funannotate annotate -i fun --tmpdir /scratch/project/adna/tmp --cpus \$SLURM_CPUS_PER_TASK --busco_db protists --no-progress --phobius $WORKDIR/phobius/phobius.results.txt --iprscan $(find $WORKDIR/ipscan -name *.proteins.fa.xml)" >  $JOB_NAME.slurm
# submit to the cluster
sbatch $JOB_NAME.slurm
...
mkdir -p /scratch/project/adna/Oyster_QX_Disease/Nanopore_sequencing/SRO_781
...
WORK_DIR=/scratch/project/adna/Oyster_QX_Disease/Comparative_Genomics # on Bunya
mkdir -p $WORK_DIR && cd $WORK_DIR

# download Ascetosporea genomes from NCBI
conda activate genomics
datasets download genome accession PRJNA1051017 --include genome,rna,protein,cds,gff3,gbff --filename Ascetosporea_genomes.zip
# download metadata
...
TARGET="/scratch/project/adna/Oyster_QX_Disease/Nanopore_sequencing/SRO_781/Galaxy_analysis/Galaxy_Flye_assembly.fasta"
...
WORK_DIR=/scratch/project/adna/Oyster_QX_Disease/Comparative_Genomics # on Bunya
mkdir -p $WORK_DIR && cd $WORK_DIR
QUERY="/scratch/project/adna/Oyster_QX_Disease/Nanopore_sequencing/SRO_781/Galaxy_analysis/Galaxy_Flye_assembly.fasta"
...
MEM=16
WALLTIME=1:00:00
WORKDIR=/scratch/project/adna/Oyster_QX_Disease/Comparative_Genomics # on Bunya
mkdir -p $WORKDIR && cd $WORKDIR
QUERY="/scratch/project/adna/Oyster_QX_Disease/Nanopore_sequencing/SRO_781/Galaxy_analysis/Galaxy_Flye_assembly.fasta"
...
OrthoFinder [@emmsOrthoFinderPhylogeneticOrthology2019] was also used to compare the transcriptomes of the 4 Ascetosporea species (using the CDs as input).

```{bash orthofinder-ascetosporea}
# setup conda environment
CONDA_NAME="genomics"
mamba install -n $CONDA_NAME orthofinder
conda activate $CONDA_NAME
#setup working folder
WORKDIR=/scratch/project/adna/Oyster_QX_Disease/Comparative_Genomics
mkdir -p $WORKDIR/PRJNA1051017 && cd $WORKDIR/PRJNA1051017

# download Ascetosporea genome datasets
datasets download genome accession PRJNA1051017 --include genome,rna,protein,cds,gff3,gbff --filename Ascetosporea_genomes.zip
unzip Ascetosporea_genomes.zip

# setup analysis folder and link input files to it
JOB_NAME="Ascetosporea-orthofinder-trans"
mkdir -p $WORKDIR/$JOB_NAME/input_transcripts && cd $WORKDIR/$JOB_NAME
find $WORKDIR/PRJNA1051017 -name "GCA_*genomic.fna" | parallel --dry-run --rpl "{target} s:.+/GCA_.+\.1_UU_(.+)_1_genomic.fna:\1:" "ln -s {//}/cds_from_genomic.fna $WORKDIR/$JOB_NAME/input_transcripts/{target}.cds.fa"
# link Msyd cds file
ln -s $(find /scratch/project/adna/Oyster_QX_Disease/QX_genome_annotation/fun/update_results/ -name *.cds-transcripts.fa) $WORKDIR/$JOB_NAME/input_transcripts/Msyd.cds.fa
...
cd \$SLURM_SUBMIT_DIR
# OrthoFinder
orthofinder -p /scratch/project/adna/tmp -d -t \$SLURM_CPUS_PER_TASK -f input_transcripts" >  $JOB_NAME.slurm
# submit to the cluster
sbatch $JOB_NAME.slurm

```

This didn't result in sufficient matches, so the same analysis was repeated at the protein level.

```{bash orthofinder-prot}
# setup conda environment
CONDA_NAME="genomics"
mamba install -n $CONDA_NAME orthofinder
conda activate $CONDA_NAME
...
# setup analysis folder and link input files to it
JOB_NAME="Ascetosporea-orthofinder"
mkdir -p $WORKDIR/$JOB_NAME/input_proteins && cd $WORKDIR/$JOB_NAME
find $WORKDIR/PRJNA1051017 -name "GCA_*genomic.fna" | parallel --dry-run --rpl "{target} s:.+/GCA_.+\.1_UU_(.+)_1_genomic.fna:\1:" "ln -s {//}/protein.faa $WORKDIR/$JOB_NAME/input_proteins/{target}.proteins.faa"
# link Msyd cds file
ln -s $(find /scratch/project/adna/Oyster_QX_Disease/QX_genome_annotation/fun/update_results/ -name *.proteins.fa) $WORKDIR/$JOB_NAME/input_proteins/MSyd.proteins.faa

# run OrthoFinder
NCPUS=16
MEM=72
WALLTIME="20:00:00"
JOB_NAME="Ascetosporea-orthofinder-prots"
# create Slurm script
echo '#!/bin/bash --login
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
cd \$SLURM_SUBMIT_DIR
# OrthoFinder
orthofinder -p /scratch/project/adna/tmp -t \$SLURM_CPUS_PER_TASK -f input_proteins" >  $JOB_NAME.slurm
# submit to the cluster
sbatch $JOB_NAME.slurm

# archive folders with lots of files and remove the folders
echo "Orthogroup_Sequences MultipleSequenceAlignments Single_Copy_Orthologue_Sequences" | parallel --dry-run "tar czfv /scratch/project/adna/Oyster_QX_Disease/Comparative_Genomics/rhizaria-orthofinder/input_proteins/OrthoFinder/Results_Jan17/{}.tar.gz /scratch/project/adna/Oyster_QX_Disease/Comparative_Genomics/rhizaria-orthofinder/input_proteins/OrthoFinder/Results_Jan17/{} && rm /scratch/project/adna/Oyster_QX_Disease/Comparative_Genomics/rhizaria-orthofinder/input_proteins/OrthoFinder/Results_Jan17/{}"

tar czf $WORKDIR/$JOB_NAME/${JOB_NAME}_Results_Jan17.tar.gz $WORKDIR/$JOB_NAME/input_proteins/OrthoFinder/Results_Jan17  

rclone copy -P --include "**/*_Results*.tar.gz" $WORKDIR/$JOB_NAME Niki_QX:General/Niki_Nenadic/Niki_PhD/SRO_QX_genomic_data/Comparative_Genomics/$JOB_NAME


```

The Ascetosporean orthologs identified by OrthoFinder were further annotated using InterProScan [@jonesInterProScanGenomescaleProtein2014; @mitchellInterPro2019Improving2019] (see setup and installation details in https://idobar.github.io/QX_bioinfo_analysis). The GO terms were extracted from the InterPro output for analysing the biological processes and molecular functions shared and unique between the species.

```{bash fun-annotate-orthologs}
CONDA_NAME="base"
# setup working directory
WORKDIR="/scratch/project/adna/Oyster_QX_Disease/Comparative_Genomics/Ascetosporea-orthofinder-prots"
cd $WORKDIR

# annotate proteins with IPscan
IPSCAN_VERSION=5.66-98.0
JOBNAME="Asceto_prots_ipscan"
mkdir -p $WORKDIR/$JOBNAME
NCPUS=16
MEM=128
WALLTIME="10:00:00"
# prepare the commands (don't forget to remove the asterisk at the end of the proteins!)
find $WORKDIR/input_proteins -maxdepth 1 -name "*.proteins.faa" | parallel --dry-run "sed 's/[*]//g' {} > \$TMPDIR/{/} ; apptainer exec -B /scratch/project/adna -B /home/ibar/scratch/tools -B $WORKDIR/$JOBNAME:/output -B \$TMPDIR:/temp $NXF_SINGULARITY_CACHEDIR/interproscan_${IPSCAN_VERSION}.sif /opt/interproscan/interproscan.sh -i /temp/{/} -d /output -dra -pa -dp -goterms -f TSV -T /scratch/project/adna/tmp -cpu \$SLURM_CPUS_PER_TASK && gawk '\$4~/PANTHER/' $WORKDIR/$JOBNAME/{/}.tsv > $WORKDIR/$JOBNAME/{/.}.panther.tsv" > $JOBNAME.cmds

# submit the job array to the cluster
sbatch -a 1-$(cat $JOBNAME.cmds | wc -l)%10 --job-name=$JOBNAME --cpus-per-task=$NCPUS --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$JOBNAME.cmds,CONDA_NAME=base  ${HOME}/bin/array.slurm

# copy to SharePoint
rclone copy -P Asceto_prots_ipscan Niki_QX:General/Niki_Nenadic/Niki_PhD/SRO_QX_genomic_data/Comparative_Genomics/Ascetosporea-orthofinder-prots/Asceto_prots_ipscan
```

The _Marteilia pararefringens_ proteins used for the phylogenetic analysis in @hiltunenthorenComparativeGenomicsAscetosporea2024 were retrieved from the protein alignments in the [FigShare repository](https://figshare.com/articles/dataset/1_Multiple_sequence_alignments/24874638) and were used to search for orthologs in the predicted proteome of _M. sydneyi_.

```{bash rhizaria-phylo}
CONDA_NAME="genomics"
mamba install -n $CONDA_NAME mafft
WORKDIR=$HOME/adna/Oyster_QX_Disease/Comparative_Genomics
JOB_NAME="rhizaria-phylo"
mkdir -p $WORKDIR/$JOB_NAME/protein_align && cd $WORKDIR/$JOB_NAME

# download protein alignments
aria2c --header 'Host: figshare.com' --user-agent 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:133.0) Gecko/20100101 Firefox/133.0' --header 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8' --header 'Accept-Language: en-AU,en-US;q=0.7,en;q=0.3' --referer 'https://figshare.com/' --header 'DNT: 1' --header 'Cookie: fig_tracker_client=49202a34-9e11-4e4b-9617-a6cf1dc5e3c1; GLOBAL_FIGSHARE_SESSION_KEY=47879e13a067258bd9f5eb31116af03453281755ad585889bf0d44f7896a326500f571dc; FIGINSTWEBIDCD=47879e13a067258bd9f5eb31116af03453281755ad585889bf0d44f7896a326500f571dc; figshare-cookies-essential=true; figshare-cookies-performance=true' --header 'Upgrade-Insecure-Requests: 1' --header 'Sec-Fetch-Dest: document' --header 'Sec-Fetch-Mode: navigate' --header 'Sec-Fetch-Site: same-origin' --header 'Sec-Fetch-User: ?1' --header 'Sec-GPC: 1' 'https://figshare.com/ndownloader/articles/24874638/versions/1' --out " $WORKDIR/$JOB_NAME/protein_align/24874638.zip" && unzip $WORKDIR/$JOB_NAME/protein_align/24874638.zip

cd $WORKDIR/$JOB_NAME/protein_align
# remove gaps from MSA
ls -1 *.mafft.fasta | parallel --dry-run --rpl "{gene} s:\.mafft\.fasta::" "sed -r 's/-//g' {} | reformat.sh in=stdin.fa out=stdout.fa fixjunk > {gene}.prots.fa"

# extract Marteilia pararefringens proteins from the MSAs
grep "Marteilia" *.mafft.fasta | cut -f1 -d: | parallel --dry-run --rpl "{gene} s:\.mafft.fasta::" "seqkit grep -n -r -p \"Marteilia\" {} | sed -r 's/>.+/>{gene}_Marteilia_pararefringens_M18/; s/-//g' | reformat.sh in=stdin.fa out=stdout.fa fixjunk > {gene}_Marteilia_pararefringens.fa"
# combine into a single multi-protein fasta
cat *Marteilia_pararefringens.fa > Marteilia_pararefringens_proteins.fa

# or concatenate all the genes from each taxa
grep ">" *.mafft.fasta | cut -f2 -d: | sed 's/>//' | sort | uniq > taxa.list 
# create a cmds file (remove any existing protein fasta files)
conda activate $CONDA_NAME
echo  "rm $WORKDIR/$JOB_NAME/protein_align/*.proteins.fa" > arrange_prots_by_taxa.cmds
parallel --dry-run --rpl "{clean_taxa} s:_N/A::g" --rpl "{gene} s:.+/(.+?)\.mafft.fasta:\1:" "seqkit grep -n -r -p \"{2}\" {1} | sed -r '/^>/s/>.+/>{1 gene}_{2 clean_taxa}/; /^>/! s/-//g' >> $WORKDIR/$JOB_NAME/protein_by_taxa/{2 clean_taxa}.proteins.fa" ::: $(find $WORKDIR/$JOB_NAME/protein_align/*.mafft.fasta) ::: $(cat $WORKDIR/$JOB_NAME/protein_align/taxa.list) >> arrange_prots_by_taxa.cmds

bash arrange_prots_by_taxa.cmds 2>&1 > arrange_prots_by_taxa.log

find $WORKDIR/$JOB_NAME/protein_by_taxa -name *.proteins.fa | parallel --dry-run "fasta-o-matic -f {} -o fixed_prots -q" > fix_prots.cmds
bash fix_prots.cmds 2>&1 > fix_prots.log  


```

OrthoFinder [@emmsOrthoFinderPhylogeneticOrthology2019] was then used to compare the set of Rhizaria proteins used in .

```{bash orthofinder-rhizaria}
# run OrthoFinder on all the rhizaria proteins (from the comparative phylogeny analysis)
# link Msyd cds file
CONDA_NAME="genomics"
WORKDIR="/scratch/project/adna/Oyster_QX_Disease/Comparative_Genomics/"
NCPUS=16
MEM=72
WALLTIME="10:00:00"
JOB_NAME="rhizaria-orthofinder-prots"
mkdir -p $WORKDIR/$JOB_NAME/input_proteins
cd $WORKDIR/$JOB_NAME
ln -s $(find /scratch/project/adna/Oyster_QX_Disease/QX_genome_annotation/fun/update_results/ -name *.proteins.fa) $WORKDIR/$JOB_NAME/input_proteins/MSyd.proteins.faa

# find $WORKDIR/rhizaria-phylo/protein_align -name *.proteins.fa | parallel "sed '/[A-Z]/s/X//g' {} > $WORKDIR/$JOB_NAME/input_proteins/{/.}.faa"
ln -sf $WORKDIR/rhizaria-phylo/protein_by_taxa/fixed_prots/*.fasta ./input_proteins/

# create Slurm script
echo 'orthofinder -p /scratch/project/adna/tmp -t $SLURM_CPUS_PER_TASK -f input_proteins' >  $JOB_NAME.slurm
# submit to the cluster
sbatch $JOB_NAME.slurm

# archive folders with lots of files and remove the folders
cd $WORKDIR/$JOB_NAME/input_proteins/OrthoFinder/Results_Jan23/ 

printf "nMultipleSequenceAlignments\nSingle_Copy_Orthologue_Sequences\nWorkingDirectory" | parallel "tar czf $WORKDIR/$JOB_NAME/input_proteins/OrthoFinder/Results_Jan23/{}.tar.gz $WORKDIR/$JOB_NAME/input_proteins/OrthoFinder/Results_Jan23/{} && rm -rf $WORKDIR/$JOB_NAME/input_proteins/OrthoFinder/Results_Jan23/{}"

tar czf $WORKDIR/$JOB_NAME/${JOB_NAME}_Results_Jan23.tar.gz $WORKDIR/$JOB_NAME/input_proteins/OrthoFinder/Results_Jan23

# use MSA-based phylogeny (specifying Telonema_subtile_merged_Tel-1 as an outgroup)
JOB_NAME="rhizaria-orthofinder-prots-msa"
echo 'orthofinder -p /scratch/project/adna/tmp -t $SLURM_CPUS_PER_TASK -fg Results_Jan23 -M msa ' >  $JOB_NAME.cmds
# submit to the cluster
# submit the job to the cluster
sbatch --job-name=$JOB_NAME --cpus-per-task=$NCPUS --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$JOB_NAME.cmds,CONDA_NAME=$CONDA_NAME ~/bin/serial_jobs_run.slurm

tar czf $WORKDIR/$JOB_NAME/${JOB_NAME}_Results_Feb09.tar.gz $WORKDIR/$JOB_NAME/Results_Feb09

```

> TODO: Look at the shared orthologues between _M. sydneyi_ and _M. pararafringens_ and find which contigs harbour them (as further evidence that they are indeed _Marteilia_ sequences). Use the comparison with the full Rhizaria dataset to infer phylogeny.

### stLFR Sequencing and Assembly

DNA sample from QX-infected oyster (NN_781) was sent to the WA State Agricultural Biotechnology Centre ([SABC](https://www.murdoch.edu.au/research/sabc)) at Murdoch University (led by Prof. Rajeev Varshney) for single tube long fragment read (stLFR) sequencing on an MGI DNBSEQ-T7 [@wangEfficientUniqueCobarcoding2019a] (see @zhangComparisonDifferentSequencing2021 for a comparison of sequencing and assembly strategies, including Nanopore and stLFR and @lagnel-hal-04009338 for a hybrid approach utilizing these sequencing technologies ). 

Sequencing data processing, including basecalling, QC and other analyses were performed on the University of Queensland *Bunya* high-performance computing (HPC) cluster (see [documentation](https://github.com/UQ-RCC/hpc-docs/tree/main)), made available through The [Queensland Cyber Infrastructure Foundation](https://www.qcif.edu.au/services/high-performance-computing) (QCIF). Additional analyses were performed in [GalaxyAU](https://usegalaxy.org.au) and Griffith University *Gowonda* HPC cluster (see [documentation](https://griffith.atlassian.net/wiki/spaces/GHCD/overview?homepageId=4030474)). 
The raw stLFR reads need to be deconvulated based on the barcodes found in the last 42bp of the reverse (R2) reads (a *whitelist* containing these barcodes can be found in the [stLFR_barcode_split repository](https://github.com/BGI-Qingdao/stLFR_barcode_split/blob/8a22b7f152b0f18a2d3db59733368cc4352cbc58/barcode_list.txt) or downloaded from [LRTK databases](https://drive.google.com/drive/folders/1XPW2avL_LZAt5yIh9tb35jZ5GfCSj7eQ)). Several pipelines have been developed to process such reads, as detailed below.


#### Assembly with Supernova
The [stLFR2Supernova](https://github.com/BGI-Qingdao/stlfr2supernova_pipeline) (or [stLFRdenovo](https://github.com/BGI-biotools/stLFRdenovo)) pipeline was used to process the raw reads, convert them to the 10X Genomics format and then assemble a draft genome using Supernova [@visendiNovoAssemblyLinked2022]. 
...
# Find barcode whitelist
WHITELIST=$(find ~/etc/tools -name "barcode_list.txt" | head -n1)
CONDA_NAME="base"
WORKDIR="/scratch/project/adna/Oyster_QX_Disease/stLFR_Murdoch"
NCPUS=24
MEM=512
WALLTIME="50:00:00"
JOB_NAME="SRO-stlfr-supernova"
mkdir -p $WORKDIR/$JOB_NAME && cd $WORKDIR/$JOB_NAME

echo "~/etc/tools/stLFRdenovo-v1.0.5/stLFRdenovo_pipeline.sh -f $WORKDIR/Oyster_781.2.R1.fq.gz $WORKDIR/Oyster_781.2.R2.fq.gz -o output -m $MEM -t $NCPUS" > $JOB_NAME.cmds
# submit the job to the cluster
sbatch --job-name=$JOB_NAME --cpus-per-task=$NCPUS --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$JOB_NAME.cmds,CONDA_NAME=$CONDA_NAME ~/bin/serial_jobs_run.slurm
# job failed with TIMEOUT error
# Continue just from the assembly step
NCPUS=24
MEM=512
WALLTIME="100:00:00"
JOB_NAME="SRO-stLFRdenovo"
echo "~/etc/tools/stLFRdenovo-v1.0.5/step_2_run_assembly.sh -f output/read.1.fq.gz.clean.gz output/read.2.fq.gz.clean.gz -o output -m $MEM -t $NCPUS" > $JOB_NAME.cmds
# submit the job to the cluster
sbatch --job-name=$JOB_NAME --cpus-per-task=$NCPUS --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$JOB_NAME.cmds,CONDA_NAME=$CONDA_NAME ~/bin/serial_jobs_run.slurm

```

An alternative for `supernova` is SLR-superscaffolder, which will require converting the stLFR reads to 10X Genomics format [@guoSLRsuperscaffolderNovoScaffolding2021]. 

##### Gap filling
After an initial `supernova` assembly any gaps in the initial draft assembly can be filled with the Oxford Nanopore generated reads. This can be done with a range of dedicated tools, such as [GABOLA](https://hub.docker.com/r/lsbnb/gabola) (or its successor [LOCLA](https://github.com/lsbnb/locla), see ), RagTag [@alongeAutomatedAssemblyScaffolding2022], [FGAP](https://github.com/pirovc/fgap) [@piroFGAPAutomatedGap2014], SAMBA [@ziminSAMBAToolUses2022], LR_Gapcloser [@xuLR_GapcloserTilingPathbased2019], PGcloser [@luPGcloserFastParallel2020], DENTIST [@ludwigDENTISTUsingLong2021] to name a few.

A recent review by @zhaoComprehensiveEvaluationGenome2024 showed that for every dataset a different gap filler performs best (with FGAP, SAMBA and DENTIST performing best in their datatests), therefore a few approaches can be utilised and compared using an assembly QC evaluation program such as QUAST [@gurevichQUASTQualityAssessment2013].

We chose to compare SAMBA (installed through [MaSuRCA](https://github.com/alekseyzimin/masurca#samba-scaffolder) Genome Assembly and Analysis Toolkit), [FGAP](https://github.com/pirovc/fgap) and [LOCLA](https://github.com/lsbnb/locla#ii-10x-genomics--pacbioont-pipeline)

First we need to prepare the Nanopore data (as detailed in Section \@ref(nanopore-sequencing-and-basecalling), but using the latest [dorado](https://github.com/nanoporetech/dorado) models)
```{bash prep-ont-data}
CONDA_NAME="ont"
conda activate $CONDA_NAME
cd ~/adna/tools
...
# link ONT raw reads
cd /scratch/project/adna/Oyster_QX_Disease/Nanopore_sequencing/SRO_781
mkdir -p pod5
find /scratch/project/adna/Oyster_QX_Disease/Nanopore_sequencing -name "*.pod5" -exec ln -s {} pod5/ \;
NCPUS=16
MEM=32
WALLTIME="5:00:00"
JOB_NAME="SRO-ont-split-pod5"

echo "pod5 view pod5 --include \"read_id, channel\" --output pod5_summary.tsv; pod5 subset pod5 --summary pod5_summary.tsv --columns channel --output pod5_split_by_channel" > $JOB_NAME.cmds
# submit the job to the cluster
sbatch --job-name=$JOB_NAME --cpus-per-task=$NCPUS --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$JOB_NAME.cmds,CONDA_NAME=$CONDA_NAME ~/bin/serial_jobs_run.slurm 
# Duplex basecalling
MINQ=7
...
JOB_NAME="SRO-ont-duplex-basecall"
find pod5_split_by_channel -name "*.pod5" | parallel --dry-run "dorado duplex --reference $WORK_DIR/Sro_reference_concatenated.fasta -x cuda:0 --min-qscore $MINQ $MODEL pod5/ --emit-fastq > ${BATCH}_SRO-mapped_reads_Q${MINQ}.bam; dorado summary -v ${BATCH}_SRO-mapped_reads_Q${MINQ}.bam > ${BATCH}_sequencing_summary.txt ; samtools view -Ofastq -f 4 ${BATCH}_SRO-mapped_reads_Q${MINQ}.bam | pbgzip -c -8 > ${BATCH}_unmapped_reads.fastq.gz; seqkit grep -nri -p  ';' ${BATCH}_unmapped_reads.fastq.gz -o ${BATCH}_duplex_unmapped_reads_Q7.fastq.gz; seqkit grep -nriv -p  ';' ${BATCH}_unmapped_reads.fastq.gz -o ${BATCH}_simplex_unmapped_reads_Q7.fastq.gz" | qsub -l select=1:ncpus=8:ngpus=1:mem=128gb,walltime=20:00:00 -N Niki_QX_1 -q gpuq2 


dorado basecaller hac <reads> ... | dorado trim --sequencing-kit <kit> > trimmed.bam
# Error correction
...
```{bash gap-fill}
CONDA_NAME="gap-fill" 
mamba create -n $CONDA_NAME masurca fgap canu # dentist-core snakemake ragtag

# download ONT data
cd /scratch/project/adna/Oyster_QX_Disease/


WORKDIR="/scratch/project/adna/Oyster_QX_Disease/stLFR_Murdoch/SRO-stLFRdenovo"
NCPUS=16
MEM=128
WALLTIME="20:00:00"
JOB_NAME="SRO-ont"
mkdir -p $WORKDIR/$JOB_NAME/ONT_data && cd $WORKDIR/$JOB_NAME


echo "samba.sh -r $WORKDIR/output/scaffolds.fasta.gz -q <long reads or another assembly used to scaffold in fasta or fastq format, can be gzipped> -d ont -m 2500 -t $NCPUS" > $JOB_NAME.cmds
# submit the job to the cluster
sbatch --job-name=$JOB_NAME --cpus-per-task=$NCPUS --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$JOB_NAME.cmds,CONDA_NAME=$CONDA_NAME ~/bin/serial_jobs_run.slurm
...
Another approach that may be more suitable for our scenario is implemented in pipelines that attempt to assemble metagenomes from the stLFR or Nanopore reads. 
...
* Hifiasm is an assembler designed for PacBio HiFi data, but the recent version can use ONT data as input. It can be used in conjuction with [HiFiasm-meta](https://github.com/xfengnefx/hifiasm-meta) to try and assemble the metagenome (see details [here](https://github.com/xfengnefx/hifiasm-meta/issues/38#issuecomment-2611655948)) and then use the stLFR data to polish it.
...
```{bash backup}
rclone copy -P -L --exclude "**work/*" --exclude "**nextflow/*" --exclude "**.nextflow*" --exclude "**/*.html" --exclude "**/*.xlsx" Oyster_QX_Disease Niki_QX:General/Niki_Nenadic/Niki_PhD/SRO_QX_genomic_data

rclone copy -P -L --ignore-checksum --ignore-size --include "**/*.html" --include "**/*.xlsx" Oyster_QX_Disease Niki_QX:General/Niki_Nenadic/Niki_PhD/SRO_QX_genomic_data
```
