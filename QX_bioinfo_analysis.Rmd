...
grep ">" rps16.mafft.fasta | sed 's/>//' > taxa.list 
parallel --dry-run --rpl "{clean_taxa} s:_N/A::g" --rpl "{gene} s:\.mafft.fasta::" "seqkit grep -n -r -p \"{2}\" {1} | sed -r 's/>.+/>{1 gene}_{2 clean_taxa}/; s/-//g' | reformat.sh in=stdin.fa out=stdout.fa fixjunk >> {2 clean_taxa}.proteins.fa" ::: $(ls -1 *.mafft.fasta) ::: $(cat taxa.list) > arrange_prots_by_taxa.cmds

bash arrange_prots_by_taxa.cmds
# concatenate all the prots for each taxa



# seqkit replace -p "(.)" -r "{fbne}_\$1"

grep ">" rps16.mafft.fasta
