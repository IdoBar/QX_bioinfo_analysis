...
knitr::opts_chunk$set(list(echo = TRUE, eval=FALSE, message=FALSE, class.source='klippy'))
fav_packages <- c('tidyverse', 'rentrez', 'taxonomizr', "juba/rmdformats", "rlesur/klippy")
...
WORK_DIR=$HOME/scratch/McDougall/Niki_QX_genome/Nanopore_sequencing
mkdir -p $WORK_DIR/blobtk_dir && cd $WORK_DIR
QUERY="Galaxy_Flye_assembly"
...
echo "source ~/.bashrc; cd $PWD; conda activate $CONDA_NAME; set -Eeo pipefail; blobtools create --fasta $QUERY.fasta --taxid 98371 --taxdump $TX_DB_DIR $WORK_DIR/blobtk_dir/$QUERY" | qsub -l select=1:ncpus=8:mem=32gb,walltime=10:00:00 -N  # --meta $QUERY.yaml \

# add diamond and BLAST hits
echo "source ~/.bashrc; cd $PWD; conda activate $CONDA_NAME; set -Eeo pipefail; blobtools add --bitscore 100 --hits $QUERY.nt_tax.outfmt6 --hits $QUERY.nr_tax.outfmt6.txt --taxrule bestsumorder --taxdump $TX_DB_DIR --hits-cols 1=qseqid,14=staxids,12=bitscore,2=sseqid,7=qstart,8=qend,11=evalue $WORK_DIR/blobtk_dir/$QUERY" | qsub -l select=1:ncpus=8:mem=32gb,walltime=10:00:00 -N add_blob_hits 
# add coverage 
echo "source ~/.bashrc; cd $PWD; conda activate $CONDA_NAME; set -Eeo pipefail; blobtools add --cov combined_QX1_QX2_on_Galaxy_Flye_assembly_minimap2.bam $WORK_DIR/blobtk_dir/$QUERY" > blobtk_add_cov.sh
qsub -l select=1:ncpus=8:mem=32gb,walltime=10:00:00 -N blobtk_add_cov blobtk_add_cov.sh 

# add BUSCo results
echo "source ~/.bashrc; cd $PWD; conda activate $CONDA_NAME; set -Eeo pipefail; blobtools add --busco $QUERY_Busco_full_table.tabular $WORK_DIR/blobtk_dir/$QUERY" > blobtk_add_busco.sh
...
tar czf $WORK_DIR/blobtk_$QUERY.tgz $WORK_DIR/blobtk_dir/$QUERY
...
```{bash backup}
rclone copy -P -L --exclude "**work/*" --exclude "**nextflow/*" --exclude "**.nextflow*" --exclude "**/*.html" Oyster_QX_Disease Niki_QX:General/Niki_Nenadic/Niki_PhD/SRO_QX_genomic_data

rclone copy -P -L --ignore-checksum --ignore-size --include "**/*.html" Oyster_QX_Disease Niki_QX:General/Niki_Nenadic/Niki_PhD/SRO_QX_genomic_data
```
