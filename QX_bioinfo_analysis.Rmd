 ...
 * Trimming of low-quality bases and removal of sequencing adaptors with Fastp v0.20.1 [@chenFastpUltrafastAllone2018]  
 * Quality assessment of sequenced reads with FastQC v0.11.9  
 * Ribosomal RNA (rRNA) removal matching against the [SILVA rRNA database](https://www.arb-silva.de/) (release 138.1) [@quastSILVARibosomalRNA2012]  
 * Transcriptome assembly using a range of tools, including Transcriptome assembly using a range of tools, including Trinity v2.9.1, SPAdes v3.14.0, Trans-ABySS v2.0.1 and SOAPdenovo-Trans v1.03, with a range of k-mers (see a comprehensive comparison of these tools at @holzerNovoTranscriptomeAssembly2019 and of k-mer selection at @haznedarogluOptimizationNovoTranscriptome2012)   
 * Collapsing redundant transcripts with [EvidentialGene](http://arthropods.eugenes.org/EvidentialGene/)   
 * Annotation of the assemblies against UniProt databases using DIAMOND v0.9.30    
 * Building a [Trinotate](https://github.com/Trinotate/Trinotate/wiki) v3.2.1 database for each assembly and its annotation  
...
chmod +x nextflow-22.11.1-edge-all # this is the only version that supports DSL1, Apptainer and has an updated Tower plugin!
```
...
In addition to the annotation performed with TransPi, the transcriptomes were annotated against the non-redundant nucleotide and protein databases of the NCBI (`nt` and `nr`, respectively, downloaded from NCBI on 26/02/2024) to achieve more accurate species-specific annotations.
...
<!-- The SRO transcriptome of non-infected (control) tissues was also assembled with the [Oyster River Protocol](https://oyster-river-protocol.readthedocs.io/en/latest/index.html) pipeline [@macmanesOysterRiverProtocol2018a]. -->
