...
The predicted proteins in the transcriptomes were further annotated using InterProScan [@jonesInterProScanGenomescaleProtein2014; @mitchellInterPro2019Improving2019] (or [FA-nf](https://github.com/guigolab/FA-nf)) to assign protein families, motifs and ontologies to assist with transcript-to-gene annotation.
...
aria2c -x 6 -c $DATA_URL && \
 ...
 md5sum -c ${IPRO_DATA}.md5 && pigz -cd $IPRO_DATA | tar xf -
...
OrthoFinder [@emmsOrthoFinderPhylogeneticOrthology2019] was used to compare the transcriptomes of the 4 Ascetosporea species (using the CDs as input).
...
printf "nMultipleSequenceAlignments\nSingle_Copy_Orthologue_Sequences\nWorkingDirectory" | parallel "tar czf $WORKDIR/$JOB_NAME/input_proteins/OrthoFinder/Results_Jan23/{}.tar.gz $WORKDIR/$JOB_NAME/input_proteins/OrthoFinder/Results_Jan23/{} "

printf "nMultipleSequenceAlignments\nSingle_Copy_Orthologue_Sequences\nWorkingDirectory" | parallel "rm -rf $WORKDIR/$JOB_NAME/input_proteins/OrthoFinder/Results_Jan23/{}"
...
Look at the shared orthologues between _M. sydneyi_ and _M. pararafringens_ and find which contigs harbour them (as further evidence that they are indeed _Marteilia_ sequences). Use the comparison with the full Rhizaria dataset to infer phylogeny.
...
```{bash backup}
rclone copy -P -L --exclude "**work/*" --exclude "**nextflow/*" --exclude "**.nextflow*" Oyster_QX_Disease Niki_QX:General/Niki_Nenadic/Niki_PhD/SRO_QX_genomic_data
```
